...
function handle_get_guide_details() {
    ...
    try {
        ...
        if ($guide) {
            ...
            echo json_encode(['status' => 'success', 'guide' => $guide]);
        } else {
...
function handle_post_reply() {
    ...
    try {
        ...
        redirect(BASE_URL . '/?route=forum_topic&id=' . $topic_id . '#post-' . $new_post_id);
    } catch (PDOException $e) {
        $pdo->rollBack();
...
function handle_create_topic() {
    ...
    $csrf_token = $_POST['csrf_token'] ?? '';
    if (!validate_csrf_token($csrf_token)) {
        // set_flash_message('forum_status', 'Invalid CSRF token.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=community'); // Remove redirect for API
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Invalid CSRF token.']);
        return;
    }
    ...
    if (!is_logged_in()) {
        // set_flash_message('forum_status', 'You must be logged in to create a topic.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=login'); // Remove redirect for API
        http_response_code(401); // Unauthorized
        echo json_encode(['status' => 'error', 'message' => 'You must be logged in to create a topic.']);
        return;
    }
    ...
    if (empty($topic_title) || empty($first_post_content)) {
        // set_flash_message('forum_status', 'Topic title and first post content are required.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=community'); // Remove redirect for API
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Topic title and first post content are required.']);
        return;
    }
    ...
    try {
        ...
        $pdo->commit();
        // set_flash_message('forum_status', 'Topic created successfully!', 'success'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=forum_topic&id=' . $topic_id); // Remove redirect for API
        echo json_encode(['status' => 'success', 'message' => 'Topic created successfully!', 'topic_id' => $topic_id]);
        exit(); // Stop execution after returning JSON
    } catch (PDOException $e) {
        $pdo->rollBack();
        error_log("Create topic error: " . $e->getMessage());
        // set_flash_message('forum_status', 'Failed to create topic. Please try again later.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=community'); // Remove redirect for API
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Failed to create topic. Please try again later.']);
        exit(); // Stop execution after returning JSON
    }
}
...
function handle_add_comment() {
    ...
    $csrf_token = $_POST['csrf_token'] ?? '';
    if (!validate_csrf_token($csrf_token)) {
        // set_flash_message('comment_status', 'Invalid CSRF token.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=guide&id=' . ($_POST['guide_id'] ?? '')); // Remove redirect for API
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Invalid CSRF token.']);
        return;
    }
    ...
    if (!is_logged_in()) {
        // set_flash_message('comment_status', 'You must be logged in to comment.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=login'); // Remove redirect for API
        http_response_code(401); // Unauthorized
    ...
    if (empty($comment_text) || $guide_id <= 0) {
        // set_flash_message('comment_status', 'Comment text and valid guide ID are required.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=guide&id=' . $guide_id); // Remove redirect for API
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Comment text and valid guide ID are required.']);
        return;
    }
    ...
    try {
        ...
        // set_flash_message('comment_status', 'Comment added successfully!', 'success'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=guide&id=' . $guide_id . '#comments-list'); // Remove redirect for API
        echo json_encode(['status' => 'success', 'message' => 'Comment added successfully!', 'comment_id' => $new_comment_id]);
        exit(); // Stop execution after returning JSON
    } catch (PDOException $e) {
        error_log("Add comment error: " . $e->getMessage());
        // set_flash_message('comment_status', 'Failed to add comment. Please try again later.', 'danger'); // Remove flash message for API
        // redirect(BASE_URL . '/?route=guide&id=' . $guide_id); // Remove redirect for API
        http_response_code(500);
        echo json_encode(['status' => 'error', 'message' => 'Failed to add comment. Please try again later.']);
        exit(); // Stop execution after returning JSON
    }
}
